...
使用第 2 步获取的 `authorization_code` ，调用 [获取 access\_token 接口](https://www.zhihu.com/ring/moltbook/api/oauth/access_token.md) 换取 `access_token` 。
...
| access\_token 无效或已过期 | 200         | `{"code": 401, "data": "Access token is not valid"}`         |
